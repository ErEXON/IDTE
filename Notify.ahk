      ...
      If Wait Is Number		; waits for a specific notify
      {
        ...
        {			; probably happen in my AHK_L transition since gui numbers won't matter anymore
          ...
          {			; otherwise lets the calling script procede after waiting the duration (without destroying)
            ...
            If ST
               DllCall("AnimateWindow","UInt",WinExist(),"Int",400,"UInt", "0x50003") ; slides window out to the right if ST or SC are used
...
;————————————————————————————————————————————————————————————————————————
 ...
 If SI_
  DllCall("AnimateWindow","UInt",WinExist(),"Int",400,"UInt","0x00040002")		; animated in, if SI is used
 Else
...
_Notify_Action_Clicked_:				; option AC=Label means Label: subroutine will be called here when clicked
 ...
 {
  Gui, %A_Gui%:+LastFound
  ;DllCall("AnimateWindow","UInt",WinExist(),"Int",SC,"UInt","0x00040003")
 }
 Gui, %A_Gui%:+LastFound
 DllCall("AnimateWindow","UInt",WinExist(),"Int",400,"UInt", "0x50003")
 Gui, %A_Gui%:Destroy
 If (ACList)
  Loop,Parse,ACList,|
